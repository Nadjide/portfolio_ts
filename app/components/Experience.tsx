...
const Experience = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="py-20 px-6 bg-white dark:bg-[#0a0a0a] transition-colors duration-300"
      id="experience"
    >
      <motion.h2
        ...
        <div>
          ...
          <div className="relative pl-6 border-l-2 border-blue-200 dark:border-blue-800/60 space-y-10">
            {formations.map((formation) => (
              <div key={formation.id} className="timeline-entry relative">
                ...
                <div className="tl-heading">
                  <h4 className="text-base font-bold text-gray-900 dark:text-white mb-0.5">{formation.title}</h4>
                  {formation.subtitle ? (
                    <p className="text-xs text-gray-500 dark:text-gray-400 italic mb-1">{formation.subtitle}</p>
                  ) : null}
                  <p className="text-sm font-medium text-blue-600 dark:text-blue-400 mb-0.5">{formation.company}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{formation.address}</p>
                </div>
                ...
                <ul className="tl-body text-sm text-gray-500 dark:text-gray-400 space-y-1.5 mb-3 mt-3">
                  {formation.achievements.map((achievement, index) => (
                    <li key={`${formation.id}-achievement-${index}`} className="flex items-start gap-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {formation.tech.map((tech) => (
                    <span
                      key={`${formation.id}-${tech}`}
                      className="tl-tag text-xs px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700"
                    >
                      {tech}
                    </span>
                  ))}
        ...
        <div>
          ...
          <div className="relative pl-6 border-l-2 border-cyan-200 dark:border-cyan-800/60 space-y-10">
            {experiences.map((experience) => (
              <div key={experience.id} className="timeline-entry relative">
                ...
                <div className="tl-heading">
                  <h4 className="text-base font-bold text-gray-900 dark:text-white mb-0.5">{experience.title}</h4>
                  <p className="text-sm font-medium text-cyan-600 dark:text-cyan-400">{experience.company}</p>
                </div>

                <p className="tl-meta text-sm text-gray-600 dark:text-gray-300 leading-relaxed mt-2 mb-3">
                  {experience.description}
                </p>

                <ul className="tl-body text-sm text-gray-500 dark:text-gray-400 space-y-1.5 mb-4">
                  {experience.achievements.map((achievement, index) => (
                    <li key={`${experience.id}-achievement-${index}`} className="flex items-start gap-2">
